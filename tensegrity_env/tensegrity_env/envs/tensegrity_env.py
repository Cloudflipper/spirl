...
class tensegrity_env(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        self,
        xml_file=os.path.join(os.getcwd(),"tensegrity_env/3prism_jonathan_steady_side.xml"),
        ...
        tendon_reset_mean = 0,
        tendon_reset_stdev = 0.001,
        tendon_max_length = -0.3,
        tendon_min_length = -0.3,
        ...
        **kwargs
        ...
        frame_skip = 20
        #print("_____------------------------",xml_file.items())
        
        MujocoEnv.__init__(
            self, xml_file, frame_skip, **kwargs,observation_space=observation_space
        )#, 
    ...
    def step(self, action):
        ...
        #print("Action:---------------------------------------",action,self.frame_skip)
        ...
        # geom_id = self.model.geom_name2id("s0")
        # pos_r01_left_end = self.data.geom_xpos[geom_id].copy()
        # geom_id = self.model.geom_name2id("s2")
        # pos_r23_left_end = self.data.geom_xpos[geom_id].copy()
        # geom_id = self.model.geom_name2id("s4")
        # pos_r45_left_end = self.data.geom_xpos[geom_id].copy()
        # left_COM_after = (pos_r01_left_end+pos_r23_left_end+pos_r45_left_end)/3
        # geom_id = self.model.geom_name2id("s1")
        # pos_r01_right_end = self.data.geom_xpos[geom_id].copy()
        # geom_id = self.model.geom_name2id("s3")
        # pos_r23_right_end = self.data.geom_xpos[geom_id].copy()
        # geom_id = self.model.geom_name2id("s5")
        # pos_r45_right_end = self.data.geom_xpos[geom_id].copy()
    ...
    def _get_obs(self):
        ...
        # geom_id_r01 = self.model.geom_name2id("r01")
        # geom_id_r23 = self.model.geom_name2id("r23")
        # geom_id_r45 = self.model.geom_name2id("r45")
        # rotation_r01 = Rotation.from_matrix(self.data.geom_xmat[geom_id_r01].reshape(3, 3)).as_quat()
        # rotation_r23 = Rotation.from_matrix(self.data.geom_xmat[geom_id_r23].reshape(3, 3)).as_quat()
        # rotation_r45 = Rotation.from_matrix(self.data.geom_xmat[geom_id_r45].reshape(3, 3)).as_quat()
    ...
    def reset_model(self):
        ...
        for i in range(50):
            ...
            #print("tendons: ",tendons,"self._tendon_max_length",self._tendon_max_length,"self._tendon_min_length: ",self._tendon_min_length)
            self.step(tendons)
