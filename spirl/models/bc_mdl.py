...
class BCMdl(BaseModel):
    ...
    def forward(self, inputs, use_learned_prior=False):
        ...
        """
        output = AttrDict()
        #print("This input:",inputs,"_______________________")
            
        output.pred_act = self._compute_output_dist(self._net_inputs(inputs))
        # with open('example.txt', 'w', encoding='utf-8') as file:
        #     print("inputs['states'].shape:",inputs['states'][0],inputs['states'][1], file=file)
        #     print("inputs['states'].shape:",inputs['states'].shape, file=file)
        #     print(output, file=file)
        #     print(dir(output), file=file)
        #     print(output.items(), file=file)
        #     print("output['pred_act'].sigma.shape:",output['pred_act'].sigma.shape, file=file)
        #     print("output['pred_act'].mu.shape:",output['pred_act'].mu.shape, file=file)
        # exit(1)
        return output
