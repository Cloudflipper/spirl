...
WANDB_ENTITY_NAME = 'cldflpr-university-of-michigan'
...
class ModelTrainer(BaseTrainer):
    ...
    def setup_device(self):
        self.use_cuda = False#torch.cuda.is_available() and not self.args.debug
        self.device = torch.device('cpu') #torch.device('cuda') if self.use_cuda else torch.device('cpu')
        if self.args.gpu != -1:
