...
class ReplayBuffer:
    ...
    def append(self, experience_batch):
        ...
        for key in self._replay_buffer:
            ...
            unique_shapes = set(shapes)  # 获取所有唯一的形状
            shape_str = ', '.join([str(shape) for shape in unique_shapes])

            if key is "done":
                self._replay_buffer[key][idxs] = np.stack(experience_batch[key]).reshape(-1, 1)
            else:
                if key == "observation":
                    for i in range(len(experience_batch[key])):
                        if isinstance(experience_batch[key][i],tuple):
                            # 如果长度为2，只保留第一项
                            experience_batch[key][i] = experience_batch[key][i][0]
    ...
    def _init(self, example_batch):
        ...
        # with open('output.txt', 'w', encoding='utf-8') as f:
        #     print(example_batch, file=f)
        for key in example_batch:
            #print(key,np.array(example_batch[key]).shape)
            example_element = example_batch[key][0]
            #print("Example Element:", example_element)
            if isinstance (example_element,tuple):
                example_element = example_element[0]
            if isinstance(example_element, bool):
            # 如果是布尔值，创建一个长度为 1 的数组
                self._replay_buffer[key] = np.empty([int(self._max_capacity), 1], 
                                                    dtype=bool)
            else:
            # 否则，使用 example_element 的 shape
