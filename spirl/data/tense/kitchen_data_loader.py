...
from spirl.excel import excel_parse
...
class D4RLSequenceSplitDataset(Dataset):
    ...
    def __init__(self, data_dir, data_conf, phase, resolution=None, shuffle=True, dataset_size=-1):
        ...
        print("self.spec.env_name: ",self.spec.env_name)
        
        #env = gym.make(self.spec.env_name)
        self.dataset = excel_parse("./data/model_trajectory_3.csv")
        # print(self.dataset)
        # exit(1)
        #self.dataset =  h5py.File('./data/converted_atomic.hdf5', 'r')
    ...
    def __getitem__(self, index):
        ...
        #print("-_--------",seq.states.shape[0],self.subseq_len)
        try:
            start_idx = np.random.randint(0, seq.states.shape[0] - self.subseq_len - 1)
        except:
            start_idx = np.random.randint(0, seq.states.shape[0] - self.subseq_len+1)#TODO: This is problamatic
        output = AttrDict(
    ...
    def _sample_seq(self):
        #print(self.seqs,self.start,self.end)
